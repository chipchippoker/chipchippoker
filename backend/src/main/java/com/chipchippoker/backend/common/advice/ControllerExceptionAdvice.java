...
public class ControllerExceptionAdvice extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException e, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return ResponseEntity.status(E400_INVALID.getStatus())
                .body(ApiResponse.error(E400_INVALID, errorMessage));
    }
    ...
    protected ResponseEntity<Object> handleMissingServletRequestParameter(MissingServletRequestParameterException e, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return ResponseEntity.status(E400_MISSING_PARAMETER.getStatus())
                .body(ApiResponse.error(E400_MISSING_PARAMETER, String.format("필수 파라미터 (%s)를 입력해주세요", e.getParameterName())));
    }
    ...
    protected ResponseEntity<Object> handleMissingServletRequestPart(MissingServletRequestPartException e, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return ResponseEntity.status(E400_MISSING_PARAMETER.getStatus())
                .body(ApiResponse.error(E400_MISSING_MULTIPART, String.format("Multipart (%s)를 입력해주세요", e.getRequestPartName())));
    }
    ...
    protected ResponseEntity<Object> handleMissingPathVariable(MissingPathVariableException e, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return ResponseEntity.status(E400_MISSING_PATH_VARIABLE.getStatus())
                .body(ApiResponse.error(E400_MISSING_PATH_VARIABLE, String.format("Path (%s)를 입력해주세요", e.getVariableName())));
    }
    ...
    private ResponseEntity<ApiResponse<Object>> handleBaseException(ChipChipPokerBaseException exception, HttpServletRequest request) {
        log.error(exception.getMessage(), exception);
