...
public class AuthController {
	...
	public ResponseEntity<ApiResponse<AuthorizationInformationResponse>> passAuthorizationCode(String authorizationCode) {
		...
		if (token == null) {
			...
			if (authService.validateExistsUser(kakaoSocialId)) {
				...
				return new ResponseEntity<>(ApiResponse.success(authorizationCodeAlreadyAccountResponse), HttpStatus.valueOf(200));
			} else {
				// 계정이 없는 유저
				AuthorizationCodeMoreInformationResponse authorizationCodeMoreInformationResponse = new AuthorizationCodeMoreInformationResponse(202, "추가 정보가 필요합니다.", token.getAccess_token());
				return new ResponseEntity<>(ApiResponse.success(authorizationCodeMoreInformationResponse), HttpStatus.valueOf(202));
			}
		}
	}
	...
	public ResponseEntity<ApiResponse<SimpleSignupResponse>> signUp(
		@Valid @RequestBody SimpleSignupRequest simpleSignupRequest, @RequestHeader(value = "kakao-access-token") String token) throws Exception {
